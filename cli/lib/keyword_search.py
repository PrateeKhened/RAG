import pickle
from .search_utils import load_movies, CACHE_DIR, DEFAULT_SEARCH_LIMIT, tokenize_text, BM25_K1, BM25_B
...
class InvertedIndex():
    ...
    def __get_avg_doc_length(self) -> float:
        ...
        for doc_id in self.docmap:
            total_tokens += self.doc_lengths[doc_id]
        return total_tokens / len(self.docmap)
    ...
    def load(self, cache_dir: str = CACHE_DIR):
        ...
        if not os.path.exists(self.doc_lengths_path):
            raise FileNotFoundError(f"Missing file: {self.doc_lengths_path}")
...
def bm25_tf_command(doc_id, term, k1=BM25_K1, b=BM25_B):
    indx = InvertedIndex() 
    indx.load() 
    return indx.get_bm25_tf(doc_id, term, k1, b)
